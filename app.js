// app.js — Mapzimus board logic

// ── Variable labels for Correlate dropdown ──────────────────────────────────
const VAR_LABELS = {
  // Elections
  rep_pct: "Republican vote % by state (T454)",
  dem_pct: "Democratic vote % by state (T454)",
  margin: "Vote margin by state (T454)",
  pct_voted_state: "Voter turnout % by state (T449)",
  ...
  // Crime
  violent_crime_rate: "Violent crime rate by state (T340)",
  murder_rate: "Murder rate by state (T340)",
  property_crime_rate: "Property crime rate by state (T340)",
  // Education
  pct_bachelors: "% with bachelor's degree by state (T254)",
  pct_hs_grad: "% high school graduates by state (T254)",
  expenditure_per_pupil: "Per-pupil school spending by state (T258)",
  avg_teacher_salary: "Avg teacher salary by state (T268)",
  teacher_count: "Teacher count by state (T268)",
  revenue_federal_pct: "% school revenue from federal govt (T258)",
  ...
  veterans_benefits: "Veterans benefits by state (T573)",
  pct_on_snap: "% households on SNAP by state (T576)",
  pct_on_medicaid: "% households on Medicaid by state (T576)",
  snap_households: "SNAP households total (T577)",
  // Births & Fertility
  birth_rate_state: "Birth rate by state (T81)",
  fertility_rate_state: "Fertility rate by state (T81)",
  ...
  nursing_homes_cpi: "Nursing homes CPI (T770)",
  // CPI & Prices
  all_items_cpi: "All-items CPI 2000–2024 (T770)",
  food_at_home_cpi: "Food at home CPI (T770)",
  eggs_cpi: "Eggs CPI (T770)",
  beef_and_veal_cpi: "Beef and veal CPI (T770)",
  poultry_cpi: "Poultry CPI (T770)",
  dairy_cpi: "Dairy products CPI (T770)",
  cereals_bakery_cpi: "Cereals and bakery CPI (T770)",
  motor_vehicle_insurance_cpi: "Motor vehicle insurance CPI (T770)",
  gasoline_cpi: "Gasoline CPI (T770)",
  housing_cpi: "Housing CPI (T770)",
  ...
  credit_card_rate_assessed: "Credit card rate — assessed interest (T1223)",
  ...
  pct_families_own_stocks: "% families owning stocks (T1207)",
  ...
  port_domestic_tons: "Port tonnage — domestic (T1122)",
  ...
  population_pct_change_2020_2025: "Population % change by country 2020–2025 (International Statistics)",
  population_pct_change_1950_2020: "Population % change US county 1950–2020 (Population PDF)",
  population_per_sq_km: "Population density per km² by country (International Statistics)",
  birth_rate: "Birth rate by country (International Statistics)",
  // Trade
  ...
  sports_attendance_12mo: "Sports attendance last 12 months (T1273)",
  // Arts
...
// ── Color helpers ─────────────────────────────────────────────────────────────
const FC = { emotional:'#ef4444', relatability:'#3b82f6', surprise:'#f59e0b', tension:'#dc2626', visual:'#22c55e', data_ready:'#a855f7', originality:'#14b8a6' };
const FL = { emotional:'Emo', relatability:'Rel', surprise:'Sur', tension:'Ten', visual:'Vis', data_ready:'Dat', originality:'Ori' };
...
function cardHTML(d, highlight = false) {
  ...
  const fmtShort = d.fmt.split('—')[0].split('(')[0].trim().replace(/\n/g, '');
...
function renderBrowse() {
  ...
  g.innerHTML = r.length ? r.map(d => cardHTML(d)).join('') : '<div class="empty">No match — try a different keyword.</div>';
}
...
function buildVarSelect() {
  ...
  // Group by section prefix in label
  const sorted = Object.keys(VAR_LABELS).sort((a, b) => VAR_LABELS[a].localeCompare(VAR_LABELS[b]));
  sorted.forEach(v => {
    const o = document.createElement('option');
    o.value = v;
    o.textContent = VAR_LABELS[v] || v.replace(/_/g, ' ');
    sel.appendChild(o);
  });
}
...
function renderCorr() {
  ...
  const hint = `${allIds.size} ideas connect to "${VAR_LABELS[v] || v.replace(/_/g, ' ')}". Blue border = uses this variable directly.`;
  document.getElementById('corr-hint').textContent = hint;
...
// ── Init ──────────────────────────────────────────────────────────────────────
document.getElementById('subl').textContent = `${D.length} ideas · ProQuest Statistical Abstract of the U.S. 2026 · all 36 PDFs scanned · @mapzimus`;
renderBrowse();
