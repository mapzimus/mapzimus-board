...
TOPIC_RULES = [
    ...
    ("science", [r'\b(space|NASA|asteroid|satellite|orbit|Mars|moon|rocket|SpaceX|ISS|telescope|JWST|cosmic|galaxy|exoplanet|Artemis|launch)\b']),
    ...
    ("humor", [
        r'(?:Human Interest)',
        r'\b(?:weird|strange|bizarre|funny|quirky|absurd|oddly|ridiculous|ironic|paradox)\b',
        r'\b(?:tier list|embarrass|dumb question|last place|passive.aggress|cow tip)\b',
        r'\b(?:weirdest|strangest|funniest|silliest|pointless|absurdly|statistically impossible)\b',
        r'\b(?:most complained|on trial|state motto|state nickname|state bird|official state)\b',
        r'\b(?:state food|state animal|state fish|state symbol|most googled|bald)\b',
        r'\b(?:napping at work|nap law|most litigious|most aggressively named)\b',
        r'(?:renamed capital|pet obesity|puppy mill|Ranked Last|absurd law|weird law)\b',
        r'(?:animal.*?trial|trial.*?animal|went bald|how many popes|pizza deliver)\b',
        r'(?:hotdog|hot dog|how many.*?lawyer|most translated|declare war on)\b',
        r'(?:war on.*?drug|war on.*?poverty|war on.*?terror)\b',
        r'(?:farthest.*?McDonald|Americas.*?official|state.*?bird|state.*?motto)\b',
    ]),
    ...
    ("economy", [r'\b(manufactur|factory|industrial|assembly|production|supply chain|automation|reshoring|offshoring|rust belt)\b']),
    ...
    ("crime", [r'\b(law|legal|court|judge|jury|attorney|lawyer|prosecutor|defendant|plaintiff|lawsuit|litigation|statute|sentenc|parole|probation|bail|incarcerat|prison|exonerat|wrongful convict|civil asset|qualified immunit|public defender|plea|warrant|injunction|ordinance|tort|conviction|inmate|solitary|death row)\b']),

    ("inequality", [r'\b(poverty|poor|impoverish|destitut|food insecur|hunger|homeless|low.income|welfare|SNAP|WIC|free lunch|food stamp|food bank|section 8|public assist|safety net|deprivation|disadvantaged)\b']),
...
def assign_topics(title, sub, section, tbl):
    ...
    if not matched:
        section_defaults = {
            ...
            'science': ['science'], 'media': ['media'], 'humor': ['humor'], 'rural': ['rural'],
            'science': ['science'], 'economy': ['economy'], 'guns': ['guns'],
            'middle_east': ['middle_east'], 'psychology': ['psychology'], 'crime': ['crime'],
            ...
            'History': ['history'], 'Crime and Law Enforcement': ['crime','crime'],
            'Law & Justice': ['crime','crime'], 'Food & Nutrition': ['food'],
            ...
            'Space & Exploration': ['science','science'], 'Rural America': ['rural'],
            'Humor & Weird Data': ['humor'],
            'Human Interest': ['humor'], 'Manufacturing & Industry': ['economy'],
            ...
            'Construction': ['housing'], 'Manufactures': ['economy'],
            ...
            'Manufactures  -  Income': ['economy','economy'],
